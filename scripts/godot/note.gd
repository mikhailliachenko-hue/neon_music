...
func _configure_visuals() -> void:
	...
	# The imported circular platform is the outline. The old rectangular frame
	# made adjacent steps merge into one card and contradicted the round shell.
	if _is_double_foot_cue():
		_build_rail_step_platforms()
...
func _configure_double_foot_rail(panel: MeshInstance3D, footprint: MeshInstance3D, border: Node3D) -> void:
	...
	var footprint_mesh := footprint.mesh.duplicate() as QuadMesh
...
func _build_rail_step_platforms() -> void:
	...
	_sync_rail_step_platforms()


func _sync_rail_step_platforms() -> void:
	var end_platform := get_node_or_null("RailEndPlatform") as Node3D
	var start_platform := get_node_or_null("RailStartPlatform") as Node3D
	var start_footprint := get_node_or_null("RailStartFootprint") as MeshInstance3D
	if end_platform != null:
		end_platform.position.x = $Footprint.position.x
		end_platform.position.z = $Footprint.position.z
	if start_platform != null and start_footprint != null:
		start_platform.position.x = start_footprint.position.x
		start_platform.position.z = start_footprint.position.z
...
func _build_icon_glyph(_panel: MeshInstance3D) -> void:
	...
	halo_mesh.size = Vector2.ONE * (HAND_ICON_SIZE * 1.22)
	halo.mesh = halo_mesh
	var halo_material := _create_icon_mask_material(icon_texture, Color(0.002, 0.006, 0.018, 0.96), 0.05)
	halo_material.render_priority = 11
	halo.material_override = halo_material
	add_child(halo)


func _create_icon_mask_material(texture: Texture2D, color: Color, energy: float, preserve_texture_color: bool = false) -> ShaderMaterial:
	...
	material.set_shader_parameter("preserve_texture_color", preserve_texture_color)

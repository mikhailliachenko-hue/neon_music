...
- `note.gd` - `RhythmNote`; visual form for foot pads and hand targets, lane position, semantic cue shape and shatter. Ordinary steps use a grounded volumetric Quaternius sci-fi puck, contact bed and mipmapped shoe decal/frame; X/Z approach motion stays locked to the road instead of vertically pulsing like an overlay. Hand hits use a faceted shared impact token with distinct left/right fist artwork. Long double-foot rails are spawned at full cached length, always have approach/judgment footprint caps, and can follow optional smooth mirrored `straight`/`outward`/`inward` trajectories while old JSON stays straight. `DOUBLE_HAND_HOLD` uses a 108%-target-width translucent volume with a visible front impact icon, then retracts at the judgment plane. Duck uses one cohesive dark glass-and-neon squashed container instead of duplicated fence geometry.

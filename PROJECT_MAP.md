...
Latest acceptance (2026-08-18): `87 passed`; a non-writing regeneration of the active 479.4-second track produces `457` renderer notes, `432` unique hit beats and `29` controlled odd-beat accents in `18` strong eight-counts with zero V4 hard errors. Jump/duck/holds/long rails remain protected. Godot editor import/parsing, note visual contracts, tunnel interaction smoke and a headless 60 FPS main-scene runtime all pass. The existing dirty `output/neon_track.json` was deliberately not overwritten during this acceptance run.
...
- Latest acceptance evidence: 87 pytest tests passed; a non-writing canonical V4.5 regeneration reports zero hard errors and 457 renderer notes with 29 bounded odd-beat accents. Visual-contract, tunnel-interaction, editor parsing and headless runtime smokes pass. The existing generated track remains user-owned until it is regenerated through Analyzer.

...
Текущий канонический трек: `assets/audio/audio.wav` (188.32 s). Пользовательские WAV из `Downloads` используются как read-only regression/calibration corpus и не заменяют канонический трек автоматически.
...
Current V4.8 acceptance (2026-08-26): `117 passed`; editor import/parsing and the full production validator pass. The active 420-beat/188.32-second track emits 89 movement events, 168 renderer notes and 7/7 runtime-safe walls with zero hard errors. Its four-beat partial tail ends in one centred `DOUBLE_PUNCH`, candidate diagnostics match the emitted sequence, no movement extends beyond the canonical grid and the former final-phrase side-asymmetry warning is gone. Two full regenerations and two frame-clock runs are byte-identical. The synthetic runtime smoke retires 3/3 walls and 2/2 holds, emits six triggers within one 60 FPS frame, and the separate D3D12 Forward+ capture produces 27 valid production-renderer frames.
